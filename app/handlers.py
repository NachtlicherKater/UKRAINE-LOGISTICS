...
async def shto(message: Message):
    await message.answer('Термин «ВТК» является аббревиатурой от "Виртуальная Транспортная Компания". Но что именно это обозначает?\nВТК - это ролевая концепция, разработанная сообществом для добавления опыта виртуальных грузоперевозок.\nОни были вокруг с начала релиза TruckersMP, и, вероятно, до этого тоже.\nВ настоящее время существуют сотни ВТК, но тогда, когда эта концепция только начиналась, их было всего несколько.\n\nКаждая ВТК, в своём роде, является уникальной в том, как она функционирует.\nЭто зависит от руководства указанной компании и от того, каким образом они хотят быть представленными как виртуальная компания.\nЧаще всего ВТК имеют фирменную покраску, которая легко узнаваема,Они склонны придерживаться цветовой схеме во всем своем брендинге, будь то веб-сайт или логотип.\nГоворя о веб-сайтах и ​​логотипах, почти у всех ВТК есть веб-сайт, на котором вы можете найти больше информации, познакомиться с командой, посмотреть некоторые фотографии и даже подать заявку на вступление!')
